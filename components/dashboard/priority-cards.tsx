...
import { useInspectionCounts, useInstallationCounts } from "@/hooks/use-dexie-data";

export function PriorityCards() {
  const { counts } = useInspectionCounts();
  const { counts: instCounts } = useInstallationCounts();
  ...
  const PRIORITIES = [
    ...
    {
      value: `${instCounts.total ? Math.round(((instCounts.ok + instCounts.attention) / instCounts.total) * 100) : 0}%`,
      label: "System Health",
